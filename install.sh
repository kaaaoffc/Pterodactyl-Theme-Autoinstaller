...
check_token() {
  ...
  if [ "$USER_TOKEN" = "kaaahost1" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di kaaahost${NC}"
    echo -e "${YELLOW}TELEGRAM : @kaaahost1${NC}"
    echo -e "${YELLOW}WHATSAPP : +6288298505716${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 25K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© kaaahost1${NC}"
    exit 1
  fi
  clear
}
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer kaaahost1   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : kaaahost1      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : kaaahost1  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by kaaahost1 ${NC}"
